...
import { Box, Button, Paper, IconButton, FormControl, InputLabel, Select, MenuItem} from '@mui/material';
...
class RecipeProject extends React.Component {
    ...
    async componentDidMount() {
        ...
        this.getOrgs();
        window.ORIBIT?.api('track', 'recipe selected: ' + this.state.recipe.Name)
        this.ga?.send('send', 'pageview', window.location.pathname + 'recipe selected: ' +  this.state.recipe.Name);
    }
    ...
    handleOpenSource() {
        ...
        window.ORIBIT?.api('track', this.state.recipe.Name + 'deployed using open source')
    }
    ...
    handleSubmit() {
        ...
        this.props.onStepChange();
        window.ORIBIT?.api('track', this.state.recipe.Name + 'deployed using fork')
      }
    ...
    render() {
        ...
        if(this.state.step === 0) {
            ...
            <Box>
                ...
                <Button onClick={this.handleUseRecipe} variant="contained">Use this recipe</Button>
            </Box>
        } else if(this.state.step === 1) {
            content =
            <Box>
                <h3>How would you like to deploy the {this.state.recipe.Name} recipe?</h3>
                <Box  sx={{ display: 'flex', flexDirection:{ xs:'column', sm:'column', md:'row'},  justifyContent: 'space-evenly', alignContent:'center', pt:2}}>
                <Paper sx={{p:2, m:2}}>
                ...
                <Paper sx={{p:2, m:2}}>
                    <Button variant="contained" sx={{mb:2}} onClick={this.incrementStep} >Fork for my Organization</Button>
